...
- **지표 게이트(do-no-harm)** — 각 경계마다 보정 강도 α∈[0,1]를 **SSIM·ΔE·L1로 자동 선택**. 실제로 이음새를
  줄이는 만큼만 적용하고, 도움 안 되는 보정(중복프레임 잡음·이미 낮은 경계·내용 점프)은 α→0으로 스스로 버림.
  과도한 crop을 부르는 오매칭 affine도 기각 → 화면을 통째로 줌하거나 색을 죽이지 않음
...
- 모드: `tight`(경계 최대 매칭, 기본) / `balanced`(클립 자연스러움 우선) / **`local`**(경계 초입만
  램프 보정, 클립 본체는 원본 유지 → 보정이 누적 안 돼 **해상도·색감 보존**; 긴 체인에 권장)
