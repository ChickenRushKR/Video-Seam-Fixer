...
> 위: `A_last`(이전 클립 끝)와 `B_first`(다음 클립 시작) — 얼핏 비슷하다. 아래 왼쪽: 두 프레임의
> 차이를 ×5로 키우면 다리·바닥·조명 등 **화면 전체 엣지가 빛난다** = 이음새의 정체(스케일·위치·색
> 어긋남, 평균차 0.032). 아래 오른쪽: 정렬 후엔 배경 엣지가 **파랗게 사라지고**(0.013) 남는 주황은
> **사람 윤곽뿐** = 정렬로 못 지우는 *진짜 피사체 움직임*, 그래서 [2부 보간](#2부--경계-모션-보간-학습)이 맡는 부분.
